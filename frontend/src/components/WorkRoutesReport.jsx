...
export default function WorkRoutesReport({ devices }) {
  ...
  const reverseGeocodeFallback = async (lat, lon) => {
    ...
    try {
      ...
      // Pausa de 350ms obligatoria para no saturar el límite gratuito de LocationIQ (2 peticiones por segundo)
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/trips?deviceId=${selectedDevice}&from=${fromISO}&to=${toISO}`, {
  ...
  const handleViewRouteMap = async (trip) => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/route?deviceId=${trip.deviceId}&from=${fromISO}&to=${toISO}`, {
        headers: { 'Authorization': `Basic ${token}`, 'Accept': 'application/json' }
      });
    ...
    <div style={{ padding: '20px', backgroundColor: '#0F172A', minHeight: '100vh', color: '#F3F4F6', fontFamily: 'Inter, sans-serif', overflowY: 'auto' }}>
      ...
      <div style={{ marginBottom: '20px', borderBottom: '1px solid #1E293B', paddingBottom: '15px' }}>
        ...
        <p style={{ margin: '5px 0 0 0', color: '#94A3B8', fontSize: '13px' }}>
          Listado cronológico de viajes y frecuencias. Haz clic en "Ver Mapa" para visualizar el trazado recorrido.
        </p>
      </div>
